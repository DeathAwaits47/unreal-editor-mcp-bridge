	...
	FString& OutError)
{
	...
	else if (NodeType.Equals(TEXT("DynamicCast"), ESearchCase::IgnoreCase) || NodeType.Equals(TEXT("Cast"), ESearchCase::IgnoreCase))
	{
		FString TargetClass = NodeParams.IsValid() ? NodeParams->GetStringField(TEXT("target_class")) : TEXT("");
		Context = TargetClass;
		NewNode = CreateDynamicCastNode(Graph, TargetClass, PosX, PosY, OutError);
	}
	...
	else if (NodeType.Equals(TEXT("VariableGet"), ESearchCase::IgnoreCase) || NodeType.Equals(TEXT("GetVariable"), ESearchCase::IgnoreCase))
	{
		...
		FString ExternalParentClassPath;
		if (NodeParams.IsValid())
		{
			NodeParams->TryGetStringField(TEXT("external_parent"), ExternalParentClassPath);
		}
		Context = VariableName;
		NewNode = CreateVariableGetNode(Graph, Blueprint, VariableName, ExternalParentClassPath, PosX, PosY, OutError);
	}
	else if (NodeType.Equals(TEXT("VariableSet"), ESearchCase::IgnoreCase) || NodeType.Equals(TEXT("SetVariable"), ESearchCase::IgnoreCase))
	{
		...
		Context = VariableName;
		NewNode = CreateVariableSetNode(Graph, Blueprint, VariableName, ExternalParentClassPath, PosX, PosY, OutError);
	}
	...
	{
		OutError = FString::Printf(TEXT("Unknown node type: '%s'. Supported: CallFunction, DynamicCast, Branch, Event, VariableGet, VariableSet, Sequence, Add, Subtract, Multiply, Divide, PrintString"), *NodeType);
		return nullptr;
	}
	...
	FString& OutError)
{
	...
	if (!TargetClass.IsEmpty())
	{
		...
		if (!FunctionOwner)
		{
			FunctionOwner = LoadObject<UClass>(nullptr, *TargetClass);
		}
		if (!FunctionOwner)
		{
			FunctionOwner = LoadClass<UObject>(nullptr, *TargetClass);
		}
		if (!FunctionOwner)
		{
	...
	FString& OutError)
{
	if (TargetClass.IsEmpty())
	{
		OutError = TEXT("target_class is required for DynamicCast");
		return nullptr;
	}

	UClass* CastTarget = FindObject<UClass>(nullptr, *TargetClass);
	if (!CastTarget)
	{
		CastTarget = LoadObject<UClass>(nullptr, *TargetClass);
	}
	if (!CastTarget)
	{
		CastTarget = LoadClass<UObject>(nullptr, *TargetClass);
	}
	if (!CastTarget)
	{
		OutError = FString::Printf(TEXT("Could not load cast target class '%s'"), *TargetClass);
		return nullptr;
	}
...
UEdGraphNode* FBlueprintGraphEditor::CreateVariableGetNode(
	...
	const FString& ExternalParentClassPath,
	int32 PosX,
	int32 PosY,
	FString& OutError)
{
	...
	UClass* ExternalParentClass = nullptr;
	if (!ExternalParentClassPath.IsEmpty())
	{
		ExternalParentClass = LoadObject<UClass>(nullptr, *ExternalParentClassPath);
		if (!ExternalParentClass)
		{
			ExternalParentClass = LoadClass<UObject>(nullptr, *ExternalParentClassPath);
		}
		if (!ExternalParentClass || !FindFProperty<FProperty>(ExternalParentClass, VarName))
		{
			OutError = FString::Printf(TEXT("External variable '%s' not found in '%s'"), *VariableName, *ExternalParentClassPath);
			return nullptr;
		}
	}
	else
	...
	if (ExternalParentClass)
	{
		GetNode->VariableReference.SetExternalMember(VarName, ExternalParentClass);
	}
	else
	{
	...
	FString& OutError)
{
	...
	if (ExternalParentClass)
	{
		SetNode->VariableReference.SetExternalMember(VarName, ExternalParentClass);
	}
	else
	{
