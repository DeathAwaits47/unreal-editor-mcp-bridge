...
export const ROUTER_TOOL_SCHEMA = {
  ...
  description: [
    ...
    "  list_anim_graph_nodes, add_anim_graph_node, set_anim_node_property, connect_anim_pose",
    "  AnimGraph pose nodes (top-level graph): list_anim_graph_nodes returns each node's GUID + pins +",
    "  Layered-Blend branch-filter bones; add_anim_graph_node node_type = use_cached_pose|save_cached_pose|",
    "  layered_bone_blend|blend_list_by_enum (optional cache_name); set_anim_node_property takes node_id +",
    "  bone_name/blend_depth/layer_index/mesh_space_rotation_blend or cache_name or property+value;",
    "  connect_anim_pose takes from_node/to_node (+optional from_pin/to_pin e.g. 'BlendPose_3').",
    ...
    "  create_material, list_nodes, add_node, set_node_value,",
    "  connect_nodes, connect_property, recompile_material",
    "  Graph editing: add_node takes node_type (Constant3Vector, ScalarParameter, TextureSample, Multiply...)",
    "  and optional node_value {value,r,g,b,a,parameter_name,texture}; it returns a node_id (GUID).",
    "  connect_nodes uses from_node, to_node, from_output, to_input (A/B for Multiply).",
    "  connect_property uses from_node + material_property (base_color, roughness, normal, emissive...).",
    ...
    "",
    'domain:"widget" (key params: widget_blueprint, widget_name, widget_class, parent_name)',
    "  ops: inspect, create_widget_blueprint, add_widget, remove_widget,",
    "  set_widget_property, set_slot_property, compile",
    "  Edits the UMG designer tree (not just the event graph).",
    "  add_widget: widget_class short UMG name (TextBlock, Button, Image, VerticalBox...)",
    "  or a /Game/... UserWidget path; parent_name defaults to the root panel.",
    "  set_widget_property: properties {PropName: value}; text takes a string, colors take {r,g,b,a}.",
    "  set_slot_property: slot {position:{x,y}, size:{x,y}, anchors:{minx,miny,maxx,maxy}, alignment:{x,y}, z_order}.",
    "  create_widget_blueprint: asset_name + package_path (default /Game/UI/), optional root_class.",
    "",
...
export const COMPACT_ROUTER_TOOL_SCHEMA = {
  ...ROUTER_TOOL_SCHEMA,
  description: "One compact Unreal editor router. Choose a domain and operation; pass domain-specific fields in params. Domains: blueprint, anim, character, enhanced_input, material, widget, asset, sequencer, world, performance, narrative. Use query/inspect operations before edits when unsure.",
};
